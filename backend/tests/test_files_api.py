...
@pytest.mark.asyncio
async def test_apply_skill_folder_rejects_target_state_changes_after_preview(monkeypatch, tmp_path):
    monkeypatch.setattr(files_api, "check_agent_access", AsyncMock())
    monkeypatch.setattr(files_api.settings, "AGENT_DATA_DIR", str(tmp_path))
    monkeypatch.setattr(skill_map, "invalidate_cache", lambda _agent_id: None)
    agent_id = uuid.uuid4()
    ...
    (existing_root / "late.txt").write_text("appeared later\n", encoding="utf-8")
    ...
    assert exc.value.status_code == 409
    assert (existing_root / "late.txt").exists()
    assert (existing_root / "SKILL.md").read_text() == "# Old\n"
