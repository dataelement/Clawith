...
def _validate_llm_token_limits(
    *,
    max_output_tokens: int | None,
    context_window_tokens: int | None,
) -> None:
    """Ensure the configured context always leaves capacity for input tokens."""
    if (
        ...
        raise HTTPException(
            status_code=422,
            detail="Max output tokens must be smaller than the context window",
        )
...
async def set_default_llm_model(
    ...
    if previous_default != model.id:
        previous_model_condition = (
            Agent.primary_model_id.is_(None)
            if previous_default is None
            else Agent.primary_model_id == previous_default
        )
        await db.execute(
            update(Agent)
            .where(Agent.tenant_id == tenant.id)
            .where(Agent.deleted_at.is_(None))
            .where(previous_model_condition)
            .values(primary_model_id=model.id)
        )
        logger.info(
...
async def update_llm_model(
    ...
    try:
        ...
        if "label" in fields_set and data.label is not None:
        ...
        if "supports_vision" in fields_set and data.supports_vision is not None:
        ...
        if "request_timeout" in fields_set:
